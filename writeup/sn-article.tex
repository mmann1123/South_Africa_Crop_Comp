...
\caption{Per-class spatial-transfer F1 for four representative models.}
\label{fig:perclass}
\par\vspace{4pt}
{\footnotesize Lucerne/medics and canola are classified well by all models; barley,
wheat, and small-grain grazing concentrate the errors. CNN-BiLSTM degrades across every
class.\par}
\end{figure}
...
The sparse-attention L-TAE-S falls between the two regimes ($\Delta=-0.17$), showing
that adding feature sparsity to a dense attention backbone partly recovers transfer.
Bars show family means; points are individual models.\par}
\end{figure}
