...
retaining the temporal-attention backbone of the L-TAE~\cite{ltae2020}. We use L-TAE-S
only in the feature-dimensionality analysis (Section~\ref{subsec:featdim}), where it
isolates the effect of \emph{soft}, learned sparsity on spatial transfer.
...
$-0.05$ (logistic regression) to $-0.31$ (CNN-BiLSTM). Second, this unevenness
reorders the models. Under in-region validation the dense temporal deep
nets look strongest — CNN-BiLSTM, TempCNN, and L-TAE all reach F1$_m\approx0.77$--$0.78$,
and conventional field-level evaluation (on which the original analysis of this
dataset relied \cite{capstone_repo}) singled out CNN-BiLSTM as the best model. On the holdout tile that conclusion
collapses: CNN-BiLSTM falls to F1$_m=0.46$ (Kappa 0.32), the \emph{largest} drop
of any model and third-worst absolute score (only the two weak TabNet variants score
lower). The same fate befalls the 3D CNN
...
CNN-BiLSTM            & field & raw & 0.77 & $-0.31$ & 0.46 & 0.32 & 0.56 & 7.73 \\
...
sparsemax) modestly improved transfer over the
dense L-TAE at matched data ($0.61$ vs.\ $0.57$ at $75\%$ of fields), consistent with
