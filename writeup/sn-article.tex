...
This study makes five contributions. First, we show that the sparse, axis-aligned feature-selection inductive bias behind the two-decade dominance of Random Forest and gradient-boosted trees in remote sensing is precisely the property that governs \emph{spatial} transfer, and that TabNet inherits it through attentive sparsemax feature masks while dense CNN, RNN, and attention temporal networks discard it and overfit region-specific signal. Second, we quantify the in-region to spatial-transfer ranking inversion across twenty-three models: the dense temporal networks that lead in-region — CNN-BiLSTM in particular, the model conventional evaluation singles out as best — become near-worst on the holdout, while parsimonious models transfer with the smallest gaps. Third, we identify field-level aggregation as a second, substitutable robustness lever, a form of variance reduction that recovers transfer for dense temporal models but is redundant for already-sparse ones. Fourth, we introduce L-TAE-S, a sparse-attention temporal encoder that grafts TabNet-style sparsemax feature masks onto the L-TAE: this single architectural change shrinks its spatial-transfer gap from $-0.20$ to $-0.17$ at the pixel level and to $-0.04$ when field-aggregated --- the smallest of any model --- making an attention network as transferable as the best tree ensembles. Finally, we characterize data efficiency through a training-set reduction experiment and provide the first evaluation of \texttt{xr\_fresh} automated time-series feature extraction in crop classification.
...
Model performance was assessed using a combination of standard classification metrics: macro-averaged F1 (our primary metric), Cohen’s Kappa, weighted F1, and log-loss (cross-entropy). These metrics were selected to evaluate not only overall accuracy but also class balance and prediction confidence across imbalanced crop types; they are motivated in Section~\ref{subsec:performance_metrics} and formally defined in Appendix~\ref{app:metrics}.
...
Each pixel was represented by a feature vector $\mathbf{x}_i \in \mathbb{R}^{174}$, formed from time-series features across ten months and six spectral bands and indices (B2, B6, B11, B12, EVI, Hue), capturing phenological signatures over time.
...
true month indices, so the May--June gap is preserved), the embedded sequence passes
through three pre-norm multi-head self-attention layers ($d_{\text{model}}=128$,
$8$ heads, feed-forward width $256$); the encoded time steps are mean-pooled and passed
to a two-layer classification head. Whereas the L-TAE summarizes the season with a single
lightweight query, here every observation attends to every other across stacked layers,
making this the most expressive attention model we evaluate --- yet, unlike the L-TAE-S
below, it contains no feature-sparsity mechanism. It therefore serves as a control that
...
TabNet-inspired channel-selection gate before the attention heads. For each of the
...
attends to a small learned subset of features (with key dimension $d_k=8$). A
TabNet-style prior with relaxation $\gamma=1.5$ discourages different heads from
re-selecting the same channels, and a sparsity penalty ($\lambda=10^{-3}$) is added to
the loss; the resulting masks (which can be time-varying) are differentiable and
interpretable, mirroring TabNet's sparsemax feature masks~\cite{tabnet_orig} while
retaining the temporal-attention backbone of the L-TAE~\cite{ltae2020}. (We use
...
Two 2D-convolutional blocks with $K$ filters each compute:
\[
...
spatial-transfer macro-F1, with the generalization gap
...
$-0.04$ (the field-level sparse-attention L-TAE-S) to $-0.26$ (CNN-BiLSTM). Second, this unevenness
reorders the models. Under in-region validation the dense temporal deep
nets look strongest — CNN-BiLSTM, TempCNN, L-TAE, and the Transformer encoder all reach F1$_m\approx0.72$--$0.78$,
and conventional field-level evaluation (on which the original analysis of this
dataset relied \cite{capstone_repo}) singled out CNN-BiLSTM as the best model: under
field-level aggregation its in-region F1$_m$ rises to $0.77$, the highest of any model
at that granularity. On the holdout tile that conclusion
collapses: CNN-BiLSTM falls to F1$_m=0.46$ (Kappa 0.32) — the \emph{largest} drop
of any model whether measured at the pixel level ($\Delta=-0.26$) or the field level
($\Delta=-0.31$), and among the lowest absolute scores. The same fate befalls the 3D CNN
patch ensemble ($-0.22$), the Multi-Channel 2D CNN ($-0.22$), and SMOTE-resampled
stacking ($-0.21$).
...
F1 (wF1), and cross-entropy (Xent) complete the picture. Bold marks the best value in
each metric column. The best transferer is a
...
TempCNN) do not yield the highest spatial-transfer scores. The \emph{Features} column
...
\caption{In-region (field-wise validation) versus spatial-transfer macro-F1
...
model is the TabNet pixel ensemble (F1$_m=0.60$, Kappa 0.54), tied on macro-F1 by both
...
tree-based field ensembles. That a sparse-attention deep net sits atop the robustness
ranking is itself evidence for the inductive-bias thesis: its TabNet-style sparsemax
gating buys exactly the transfer stability the dense temporal nets lack. Notably, the robust field ensembles in the upper-middle of the
...
a practitioner who selected a model by in-region validation might have
...
$-0.07$, TempCNN's gap narrows from $-0.21$ to $-0.14$, and CNN-BiLSTM — the worst
pixel-level transferer — recovers most of all when field-trained, its gap halving from
$-0.26$ to $-0.14$ (F1$_m$ $0.46\rightarrow0.51$). The sparse-attention L-TAE-S follows
...
generalization accuracy. TabNet, run at the field level with both raw field-averaged and
\texttt{xr\_fresh} inputs, behaves consistently: moving its raw inputs from pixel to field
level costs F1$_m$ $0.60\rightarrow0.43$ (the aggregation/sample-volume penalty), and the
...
architectures evaluated at both granularities. The pixel input is raw reflectance for
the deep models and per-pixel \texttt{xr\_fresh} statistics for the tree learners; the
\emph{Field input} column gives the field-level representation --- \texttt{xr\_fresh}
...
\caption{Spatial-transfer macro-F1 at pixel versus field level. Field-level aggregation
recovers transfer for dense temporal models (L-TAE, CNN-BiLSTM) but collapses the
...
\caption{Spatial-transfer macro-F1 as the number of training fields is reduced to
$75\%$, $50\%$, and $25\%$. Robust models (TabNet, logistic regression, and the
sparse-attention L-TAE-S) are nearly flat.}
\label{fig:reduction}
\end{figure}
...
($0.61$ vs.\ $0.57$ at $75\%$ of fields), consistent with
\emph{soft}, learned sparsity helping. \emph{$L_2$ regularization} of the tree models
...
\caption{Per-class spatial-transfer F1 for four representative models. Lucerne/medics and
...
backbone, or model size --- as the property that governs generalization.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{inductive_bias_gap.pdf}
\caption{Generalization gap ($\Delta$ macro-F1, spatial-transfer minus in-region)
...
overfit the training region. The sparse-attention L-TAE-S falls between the two regimes
($\Delta=-0.17$), showing that adding feature sparsity to a dense attention backbone
partly recovers transfer. Bars show family means; points are individual models.}
\label{fig:bias}
\end{figure}
...
deep models (ST F1$_m$ 0.55–0.56) while training in under two minutes, versus
...
paradigm (Figure~\ref{fig:perclass}), and they concentrate the spatial-transfer loss.
...
This study evaluated twenty-three classical, deep learning, and patch-based crop
classifiers in the Western Cape of South Africa under two protocols: conventional
in-region field-wise validation within the training region, and a spatially disjoint
...
(macro-F1 0.77); under spatial transfer CNN-BiLSTM suffers the largest degradation of
...
Controlled architectural comparisons support this reading: a plain Transformer encoder
with far greater attention capacity transfers no better than a lightweight L-TAE,
isolating sparse feature selection rather than capacity as the cause, and grafting a
sparsemax feature gate onto the L-TAE (our L-TAE-S) yields the smallest transfer gap of
any model. Field-level aggregation provides a second, substitutable robustness lever —
...
spatial-transfer performance, not just in-region validation, because the
...
More broadly, the conditions that make the Western Cape difficult — scarce and uneven
...
in this setting is therefore not a niche stress test but a more realistic account of how
remote sensing performs in the real world. Evaluating models the way they are actually
deployed --- applied to regions where no labels were collected --- and favouring
parsimonious, data-efficient classifiers that hold up under that transfer is, we argue,
what will make remote sensing deliver in precisely the settings where it matters most.
