...
modelling volumetric relationships jointly across spectral bands, spatial
...
$\sim$2{,}415 per-field predictions with replacement $B=10{,}000$ times and recompute
macro-F1 (and per-class F1) on each replicate, reporting the $2.5$th--$97.5$th percentile
...
macro-F1, which is precisely why we report five-seed ensembles rather than individual runs
throughout. Both sources are small relative to the ranking effects we report: every
field-bootstrap CI spans only $\approx\pm0.02$--$0.03$ macro-F1, so we treat ST F1$_m$
...
\midrule
TabNet                & pixel & raw & 0.73 & $-0.13$ & \textbf{0.60} & .58--.63 & \textbf{0.54} & 0.70 & \textbf{4.42} \\
...
TabNet                & field & \texttt{xr\_fresh} & 0.51 & $-0.15$ & 0.37 & .34--.40 & 0.25 & 0.53 & 6.13 \\
\bottomrule
\end{tabular}
...
$10{,}000$ resamples of the 2{,}415 holdout fields (Section~\ref{subsec:uncertainty}).
...
in-region), with \texttt{xr\_fresh} marginally lower on macro-F1 but marginally better on
...
with our own. The same tree-favouring ordering holds under a far larger,
...
transfer degrading to a macro-F1 near $0.45$; their benchmark contains tree learners
...
(Table~\ref{tab:pixfield}). These observations point to two distinct, partly substitutable pathways to
...
expensive models (TabTransformer, the patch CNNs, the pixel-level Transformer) do
...
TabTransformer         & field & \texttt{xr\_fresh} & 7.4\,h \\
\midrule
...
TabNet                 & pixel & raw & 61\,min\textsuperscript{$\dagger\ddagger$} \\
...
Multi-Channel CNN      & patch & raw & 38\,min\textsuperscript{$\dagger$} \\
\bottomrule
\end{tabular}
