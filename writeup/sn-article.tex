...
Optuna was then used to tune XGBoost over 75 trials, searching over the number of trees, tree depth, learning rate, row and column subsampling fractions, minimum split loss, minimum child weight, and $L_1$ and $L_2$ regularization strengths, with the objective of maximizing the weighted F1 score via 5-fold stratified cross-validation. The best configuration (Trial 32) is detailed in Table~\ref{tab:top5_trials}, which also achieved a validation accuracy of 76.61\%.
...
parameter. A class-balanced random sampler further
...
\centering
\scriptsize
\caption{In-region (field-wise validation) versus spatial-transfer (ST) macro-F1
...
a practitioner who selected a model by in-region validation would have
...
essentially neutral under spatial transfer — XGBoost moves from F1$_m=0.54$ to $0.56$ and
...
\begin{table}[htbp]
\centering
\caption{Spatial-transfer (ST) macro-F1 (and gap $\Delta$) at pixel versus field level for
...
ST F1$_m$ negligibly (within $\pm0.01$). Together these show that the high feature
