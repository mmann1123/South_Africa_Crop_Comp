...
The labeled data are organized into Sentinel-2 UTM grid tiles (zone 34S). Two adjacent tiles (\texttt{34S\_19E\_258N} and \texttt{34S\_19E\_259N}) form the training region, comprising 4{,}150 labeled fields covering 622~km\textsuperscript{2} of cultivated area (mean field size 15.0~ha, median 11.0~ha). A third tile (\texttt{34S\_20E\_259N}), immediately east of and adjacent to the training tiles, is reserved as a holdout region for out-of-sample evaluation and contains 2{,}417 labeled fields over 282~km\textsuperscript{2} (mean 11.7~ha, median 6.9~ha). The tiles do not overlap, so no pixel or field is shared between training and holdout, and transfer to the holdout must contend with different soils, field geometries, and acquisition conditions. Because the holdout nonetheless lies within the same agroecological zone and growing-season calendar as the training tiles, this is a relatively \emph{local} spatial shift; the generalization gaps we report should accordingly be read as a conservative lower bound on the degradation expected under transfer across regions or climates (Section~\ref{sec:discussion}). Unless otherwise noted, ``in-region'' results refer to a field-wise (\texttt{fid}-stratified) train/validation/test split within the training region, while ``out-of-sample'' or ``holdout'' results refer to predictions on the disjoint tile.
...
In the classical approach, we automated feature extraction using the \texttt{xr\_fresh} toolkit to rapidly compute a comprehensive set of statistical and temporal features from each 10-month spectral time series \cite{mann2023,mann2025}. To address class imbalance, we applied SMOTE. Several classifiers such as Logistic Regression, Random Forest, LightGBM, and XGBoost were then evaluated.
...
Table~\ref{tab:features} lists the time-series features extracted from each pixel's 10-month spectral profile \cite{mann2023}.
...
manual feature engineering required by classical approaches~\cite{maxwell2019,zhu2017}.
...
pixel-level models are trained on millions of samples, the number of statistically
...
field (\texttt{fid}) level, so no field's pixels straddle two partitions; this keeps
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{tabnet.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{cnn_hyper.pdf}
...
Two Conv2D blocks with $K$ filters each compute:
\[
...
weighted F1, the support-weighted mean of per-class F1; and log-loss
...
reorders the models. Under in-region validation the dense temporal deep
...
TempCNN) do not yield the highest out-of-sample scores. The \emph{Features} column
marks whether a model is fed the automated \texttt{xr\_fresh} time-series statistics
(174 per field) or raw reflectance; all \texttt{xr\_fresh} models are field-level
classical learners.}
\label{tab:gap}
...
\toprule
 & & & \textbf{In-reg} & & \multicolumn{4}{c}{\textbf{Out-of-sample (holdout)}} \\
...
\midrule
\textbf{TabNet}       & \textbf{pixel} & \textbf{raw} & \textbf{0.73} & $-0.13$ & \textbf{0.60} & \textbf{0.54} & \textbf{0.70} & \textbf{4.42} \\
...
Voting                & field & \texttt{xr\_fresh} & 0.69 & $-0.12$ & 0.56 & 0.47 & 0.66 & 4.82 \\
...
Stacking              & field & \texttt{xr\_fresh} & 0.64 & $-0.09$ & 0.55 & 0.47 & 0.67 & 5.01 \\
...
model is the TabNet pixel ensemble (F1$_m=0.60$, Kappa 0.54), and the most
...
field ensembles. Notably, the robust field ensembles in the upper-middle of the
table (XGBoost, Voting, LightGBM, Stacking) are precisely the models fed the
...
well, whereas the deep nets that learn from raw sequences do not. A majority-vote
ensemble over all models reaches F1$_m=0.60$
...
in-region ceiling but improving transfer. TabNet is the informative exception:
...
field-level training starves it of the per-pixel sample volume it exploits (millions of
...
A second comparison is embedded in the same table. For the tree learners, the
field-level model is fed \texttt{xr\_fresh} statistics while the pixel-level model is
fed raw reflectance, so their pixel$\rightarrow$field change is exactly the effect of
swapping raw inputs for the automated \texttt{xr\_fresh} representation. That change is
essentially neutral out of sample — XGBoost moves from F1$_m=0.54$ to $0.56$ and
LightGBM stays at $0.56$ — even though \texttt{xr\_fresh} raises the in-region score
(e.g.\ XGBoost $0.64\rightarrow0.70$) and therefore widens the gap. The 174 engineered
features thus let a tree fit the training region more tightly without transferring any
better; the value of \texttt{xr\_fresh} here is operational — a single compact vector
per field, trained in minutes without handling millions of pixels — rather than a gain
in generalization accuracy. TabNet, uniquely, was run with \emph{both} field
representations and lets us separate the two effects: moving its raw inputs from pixel
to field level costs F1$_m$ $0.60\rightarrow0.43$ (the aggregation/sample-volume
penalty), and substituting \texttt{xr\_fresh} for the raw field-averaged input costs a
further $0.43\rightarrow0.37$. So for TabNet neither aggregation nor \texttt{xr\_fresh}
helps — consistent with the conclusion that \texttt{xr\_fresh} buys operational
convenience rather than transfer accuracy.

\begin{table}[htbp]
\centering
\caption{Out-of-sample macro-F1 (and gap $\Delta$) at pixel versus field level for
architectures evaluated at both granularities. The pixel input is raw reflectance in
every case; the \emph{Field input} column gives the field-level representation —
either the automated \texttt{xr\_fresh} statistics or the field-averaged raw time
series (raw-avg). For the tree learners (XGBoost, LightGBM) the pixel$\rightarrow$field
change is therefore precisely a raw$\rightarrow$\texttt{xr\_fresh} change. Field
aggregation helps the dense temporal nets and is roughly neutral for the tree models,
but degrades TabNet.}
\label{tab:pixfield}
\renewcommand{\arraystretch}{1.1}
...
\textbf{Architecture} & \textbf{OOS F1$_m$} & \textbf{$\Delta$} & \textbf{OOS F1$_m$} & \textbf{$\Delta$} & \textbf{Field input} \\
\midrule
...
LightGBM & 0.56 & $-0.06$ & 0.56 & $-0.12$ & \texttt{xr\_fresh} \\
\bottomrule
\end{tabular}
\end{table}
...
sparse, tree-like inductive bias baked into the model (trees, TabNet),
which confers transfer intrinsically. The second is variance reduction
...
little to exploit and the weakest out-of-sample performance of any paradigm.
...
would produce — our numbers are best read as a conservative lower bound. That even
...
\]
Log-loss (cross-entropy), with one-hot labels $y_{i,c}$ and predicted
