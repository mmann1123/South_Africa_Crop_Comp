...
Model performance was assessed using a combination of standard classification metrics: macro-averaged F1 (our primary metric), Cohen’s Kappa, weighted F1, and log-loss (cross-entropy). These metrics were selected to evaluate not only overall accuracy but also class balance across imbalanced crop types; they are motivated in Section~\ref{subsec:performance_metrics} and formally defined in Appendix~\ref{app:metrics}.
...
\caption{Architecture of the sparse-attention L-TAE-S. Each ensemble member embeds the
per-pixel Sentinel-2 time series, adds month-aware positional encoding, and --- before the
multi-head temporal attention --- applies a TabNet-style \emph{sparse feature gate}
(highlighted): a per-head \texttt{sparsemax} mask selects a small subset of embedding
channels, with a relaxation prior ($\gamma=1.5$) discouraging heads from re-selecting the
same channels. This single block, which grafts a tree-like sparse feature-selection
inductive bias onto the attention backbone, is the only architectural difference from the
L-TAE. Five seeds are ensembled and aggregated to field level.}
\label{fig:ltae_s}
\end{figure}
...
(Xent), an error-penalizing log-loss computed on hard (one-hot) predictions rather than
calibrated probabilities (lower is better; defined in Appendix~\ref{app:metrics}).
...
\]
Cross-entropy (Xent). We report the value returned by scikit-learn's \texttt{log\_loss}
applied to one-hot--encoded labels. Our pipeline scores \emph{hard} class predictions
rather than calibrated probabilities, so the predicted distribution $\hat{q}_{i,c}$ is the
one-hot indicator of the predicted label clipped to $[\epsilon, 1-\epsilon]$ with
$\epsilon=10^{-7}$:
\[
...
\]
where $y_{i,c}$ is the one-hot true label and $\hat{y}_i$ the predicted class. Because the
predictions are hard assignments, each misclassification contributes a fixed
$\approx-\ln\epsilon$ while a correct one contributes $\approx 0$; Xent is therefore a
bounded, monotone transform of the error rate rather than a measure of probabilistic
calibration, and we report it only as a secondary companion to F1 and Kappa.
