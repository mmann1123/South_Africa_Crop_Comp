...
for r in range(1, n + 1):
    ax.text(-0.80, r, f"#{r}", ha="right", va="center", fontsize=9.5,
            color="#888")

# node-value note stays just above the grey panel
ax.text(0.5, n + 0.5, "node value = macro-F1", ha="center", va="center",
        fontsize=9.5, style="italic", color="#999")

# --- light-grey panel behind the takeaway text + legend ----------------------
from matplotlib.patches import FancyBboxPatch
panel = FancyBboxPatch((-0.72, n + 0.78), 2.50, 2.55,
                       boxstyle="round,pad=0.0,rounding_size=0.10",
                       mutation_aspect=0.22,
                       transform=ax.transData, zorder=0,
                       facecolor="#f2f2f2", edgecolor="#d9d9d9", linewidth=1.0)
ax.add_patch(panel)
...
# legend (inside the panel; arrows encode direction, star marks our model)
handles = [
...
ax.legend(handles=handles, loc="center", bbox_to_anchor=(0.5, n + 2.75),
          bbox_transform=ax.transData, ncol=2, frameon=False, fontsize=10.5,
          handlelength=1.6, columnspacing=1.8, labelspacing=0.7)

ax.set_xlim(-1.02, 1.82)
ax.set_ylim(n + 3.55, 0.05)   # inverted: rank 1 at top; room for panel below
ax.axis("off")
fig.tight_layout()
