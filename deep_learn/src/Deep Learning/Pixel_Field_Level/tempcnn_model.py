"""
TempCNN (Temporal Convolutional Network) for crop classification.

Applies 1D convolutions along the temporal dimension to automatically learn
temporal and spectral features from chronologically-ordered Sentinel-2 time series.

Based on Pelletier et al. (2019) "Temporal Convolutional Neural Network for the
...
Output: models/tempcnn_seed_*.pt + preprocessing artifacts
"""
...
class TempCNN(nn.Module):
    """Temporal Convolutional Network adapted for short (10-step) sequences.

    Uses kernel_size=3 (appropriate for 10 time steps vs original k=5 for 23+ steps),
    deeper channels, and a proper dense head.
    """

    def __init__(self, in_channels=6, num_classes=5, dropout=0.3):
        super().__init__()
        # Conv blocks — kernel_size=3 is better suited for 10-step sequences
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)        # (B, 128, 10)
        x = self.conv3(x)        # (B, 256, 10)
...
def main():
    t0 = time.time()
    print(f"=== TempCNN Training === Device: {device}")
    ...
    for seed in SEEDS:
        ...
        torch.save(model.state_dict(), model_path)  # initial save so file always exists
    ...
    joblib.dump(scaler, os.path.join(MODEL_DIR, "tempcnn_scaler.joblib"))
    joblib.dump(le, os.path.join(MODEL_DIR, "tempcnn_label_encoder.joblib"))
    joblib.dump(feature_cols, os.path.join(MODEL_DIR, "tempcnn_feature_columns.joblib"))
    ...
    # Report
    report = ModelReport("TempCNN Temporal Conv")
