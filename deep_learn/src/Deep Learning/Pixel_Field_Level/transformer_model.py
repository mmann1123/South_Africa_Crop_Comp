"""
Transformer encoder (Rußwurm & Körner 2020-style) for crop classification.

A plain multi-head self-attention Transformer encoder applied to pixel-level
Sentinel-2 time series. Unlike L-TAE (a single lightweight attention query),
this stacks standard TransformerEncoder layers with full self-attention across
time steps, then mean-pools over time before classification. Included as the
canonical pixel-level Transformer baseline for satellite image time series.
...
Output: models/transformer_seed_*.pt + preprocessing artifacts
"""
...
class SITSTransformer(nn.Module):
    """Plain multi-head self-attention Transformer encoder for SITS classification."""
    ...
    def __init__(self, in_channels=6, d_model=D_MODEL, n_head=N_HEAD, n_layers=N_LAYERS,
        ...
        # Standard Transformer encoder stack (pre-norm for training stability)
        ...
        # Output MLP (matches L-TAE head for fair comparison)
    ...
    def forward(self, x):
        ...
        x = x.mean(dim=1)       # temporal mean pooling -> (B, d_model)
        x = self.mlp(x)         # (B, 128)
...
def main():
    t0 = time.time()
    print(f"=== Transformer Encoder Training === Device: {device}")
    ...
    for seed in SEEDS:
        ...
        model = SITSTransformer(in_channels=N_BANDS, num_classes=num_classes).to(device)
    ...
    joblib.dump(le, os.path.join(MODEL_DIR, "transformer_label_encoder.joblib"))
    joblib.dump(feature_cols, os.path.join(MODEL_DIR, "transformer_feature_columns.joblib"))
    ...
    # Report
    report = ModelReport("Transformer Encoder (SITS)")
    report.set_hyperparameters({
        "architecture": "Transformer encoder (multi-head self-attention, mean-pooled)",
        "d_model": D_MODEL, "n_head": N_HEAD, "n_layers": N_LAYERS, "dim_feedforward": DIM_FF,
