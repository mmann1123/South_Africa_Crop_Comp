"""
TempCNN field-level ensemble inference on holdout test data (34S_20E_259N).

Uses 5-seed TempCNN models trained on field-aggregated temporal data.
...
Output: predictions_tempcnn_field.csv (field-level)

Required artifacts in models/:
  - tempcnn_field_seed_{42,101,202,303,404}.pt
  - tempcnn_field_scaler.joblib
  - tempcnn_field_label_encoder.joblib
  - tempcnn_field_feature_columns.joblib
"""
...
OUTPUT_CSV = os.path.join(SCRIPT_DIR, "predictions_tempcnn_field.csv")
...
def main():
    print(f"=== TempCNN Field-Level Ensemble Inference === Device: {device}")

    # Check artifacts
    for artifact in ["tempcnn_field_scaler.joblib", "tempcnn_field_label_encoder.joblib", "tempcnn_field_feature_columns.joblib"]:
        if not os.path.exists(os.path.join(MODEL_DIR, artifact)):
            print(f"Error: Missing {artifact}. Run tempcnn_field.py first.")
            return

    feature_cols = load(os.path.join(MODEL_DIR, "tempcnn_field_feature_columns.joblib"))
    scaler = load(os.path.join(MODEL_DIR, "tempcnn_field_scaler.joblib"))
    le = load(os.path.join(MODEL_DIR, "tempcnn_field_label_encoder.joblib"))
    ...
    for seed in SEEDS:
        ...
        model.eval()
        print(f"  Loaded tempcnn_field_seed_{seed}.pt")
