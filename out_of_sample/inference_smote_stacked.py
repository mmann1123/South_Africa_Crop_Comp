"""
SMOTE Stacked Ensemble inference on holdout test data (34S_20E_259N).

Input: combined_test_features.parquet (field-level)
Output: predictions_smote_stacked.csv
"""
...
# Model artifacts (saved by SMOTE_meta.py into MODEL_DIR)
MODEL_PATH = os.path.join(MODEL_DIR, "stacked_model_v1.joblib")
IMPUTER_PATH = os.path.join(MODEL_DIR, "imputer.joblib")
SCALER_PATH = os.path.join(MODEL_DIR, "scaler.joblib")
LABEL_ENCODER_PATH = os.path.join(MODEL_DIR, "label_encoder.joblib")

# Output
OUTPUT_CSV = os.path.join(SCRIPT_DIR, "predictions_smote_stacked.csv")


def main():
    print("=== SMOTE Stacked Ensemble Inference ===")

    for path, name in [
        (TEST_PARQUET, "Test parquet"),
        (MODEL_PATH, "Stacked model"),
    ...
    # Predict
    print("Running SMOTE Stacked prediction...")
