"""
LightGBM (Optuna-tuned) inference on holdout test data (34S_20E_259N).
...
from config import COMBINED_TEST_FEATURES_PATH, LGBM_TUNER_DIR

# Input
TEST_PARQUET = COMBINED_TEST_FEATURES_PATH

# Model artifacts
MODEL_PATH = os.path.join(LGBM_TUNER_DIR, "final_lgbm_model.joblib")
IMPUTER_PATH = os.path.join(LGBM_TUNER_DIR, "imputer.joblib")
SCALER_PATH = os.path.join(LGBM_TUNER_DIR, "scaler.joblib")
LABEL_ENCODER_PATH = os.path.join(LGBM_TUNER_DIR, "label_encoder.joblib")

# Output
OUTPUT_CSV = os.path.join(SCRIPT_DIR, "predictions_lgbm.csv")


def main():
    print("=== LightGBM (Optuna) Inference ===")
