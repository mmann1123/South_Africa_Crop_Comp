"""Train L-TAE (field-level) with a reduced fraction of training fields.

5-seed ensemble with averaged logits. Trains on field-aggregated temporal data.

Usage:
    python train_ltae_field.py --fraction 0.50 --output-dir models/ltae_field/frac_0.50
"""
...
def main():
    ...
    print(f"=== L-TAE Field Training === Fraction: {fraction}, Device: {device}")
    ...
    for seed in SEEDS_ENSEMBLE:
        ...
        best_val_f1 = 0.0
        patience_counter = 0
        model_path = os.path.join(args.output_dir, f"ltae_field_seed_{seed}.pt")
