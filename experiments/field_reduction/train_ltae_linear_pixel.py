"""Train L-TAE-Lin (pixel-level) with a reduced fraction of training fields.

L-TAE-Lin = L-TAE with a single linear classification head (no MLP decoder)
and balanced cross-entropy loss (sklearn-style inverse-frequency weights).
5-seed ensemble with averaged logits. Field-level evaluation via majority vote.

Usage:
    python train_ltae_linear_pixel.py --fraction 0.50 --output-dir models/ltae_linear_pixel/frac_0.50
"""
...
from models_arch import (
    LTAELinear, TemporalDataset,
    get_chrono_feature_cols, compute_balanced_ce_weights,
...
def main():
    ...
    print(f"=== L-TAE-Lin Pixel Training === Fraction: {fraction}, Device: {device}")
    ...
    # Balanced cross-entropy loss (sklearn-style inverse-frequency weights)
    weights = compute_balanced_ce_weights(y_train, num_classes).to(device)
    ...
    for seed in SEEDS_ENSEMBLE:
        ...
        model = LTAELinear(in_channels=N_BANDS, d_model=128, n_head=16, d_k=8,
        ...
        best_val_f1 = 0.0
        patience_counter = 0
        model_path = os.path.join(args.output_dir, f"ltae_linear_seed_{seed}.pt")
