"""Train L-TAE-S (field-level) with a reduced fraction of training fields.

L-TAE-S = L-TAE with Sparse Channel Attention (TabNet-inspired feature gating).
Field-level variant: per-pixel temporal series are averaged within each field before
training, mirroring train_ltae_field.py but with the sparse-gated LTAESparse backbone.
5-seed ensemble with averaged logits.

Usage:
    python train_ltae_sparse_field.py --fraction 1.0 --output-dir models/ltae_sparse_field/frac_1.00
"""
...
from models_arch import (
    ...
    train_epoch_sparse, evaluate_sparse, get_device, T_SEQ, N_BANDS,
)
...
def main():
    ...
    df[feature_cols] = df[feature_cols].fillna(0)
    print(f"Field-level shape: {df.shape}")
    ...
    for seed in SEEDS_ENSEMBLE:
        ...
        best_val_f1 = 0.0
        patience_counter = 0
        model_path = os.path.join(args.output_dir, f"ltae_sparse_field_seed_{seed}.pt")
        ...
        print(f"  Seed {seed}: test_acc={accuracy_score(test_labels, preds):.4f}")
    ...
    # Ensemble evaluation (each sample is already a field)
