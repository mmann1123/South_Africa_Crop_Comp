"""Train FastTabNet (pixel-level) with a reduced fraction of training fields.

Custom PyTorch TabNet with L-TAE speed optimizations (AMP, large batch, AdamW).
Operates on flat 60-dim input (10 months x 6 bands) with sparse feature selection.
5-seed ensemble with averaged logits. Field-level evaluation via majority vote.

Usage:
    python train_fasttabnet_pixel.py --fraction 0.50 --output-dir models/fasttabnet_pixel/frac_0.50
"""
...
from models_arch import (
    FastTabNet, FlatDataset, WeightedFocalLoss,
...
# FastTabNet architecture (matches original TabNet dimensions)
N_STEPS = 5
N_D = 64
N_A = 64
GAMMA = 1.5
...
def main():
    ...
    print(f"=== FastTabNet Pixel Training === Fraction: {fraction}, Device: {device}")
    print(f"  n_steps={N_STEPS}, n_d={N_D}, n_a={N_A}, gamma={GAMMA}, "
          f"n_indep={N_INDEPENDENT}, vbs={VIRTUAL_BATCH_SIZE}, "
    ...
    # Datasets (flat — no temporal reshape)
    ...
    for seed in SEEDS_ENSEMBLE:
        ...
        model = FastTabNet(
            in_dim=in_dim, n_d=N_D, n_a=N_A, n_steps=N_STEPS,
            gamma=GAMMA, n_shared=N_SHARED, n_independent=N_INDEPENDENT,
        ...
        best_val_f1 = 0.0
        patience_counter = 0
        model_path = os.path.join(args.output_dir, f"fasttabnet_seed_{seed}.pt")
        ...
        for epoch in range(N_EPOCHS):
            ...
            if (epoch + 1) % 10 == 0 or epoch == 0:
                print(f"  Epoch {epoch+1}/{N_EPOCHS} - "
                      f"loss={train_loss:.4f}, sparse={sparse_loss:.4f}, "
                      f"val_f1={val_f1:.4f}, {time.time()-t_ep:.1f}s")
        ...
        test_logits, test_labels = evaluate_sparse(model, test_loader, device)
